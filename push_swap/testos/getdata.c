...
/*per cada int crea una nova peça posant-hi el game_num, _________ canviar nom???*/
#include <stdio.h>
...
t_stack	*getdata(int argc, char **argv)
{
	...
	if (!data)
		return (0);
	printf("abans d'omplir el vector amb l'imput\n");
	i = 1;
	while (i < argc)
	{
		if (!is_int(argv[i]))
	...
	printf("despres d'omplir el vector, abans de fill stack\n");
	
	stack = malloc(sizeof(t_stack));

	stack = fill_stack(data, argc - 1);
	
	printf("despres de fill stack\n");

	printf("\nimprimim l'stack:");
	print_pieces(stack);
	printf("la llargada de l'stack és %d\n", stack->len);
